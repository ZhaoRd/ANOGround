...
4. Try a diffrent usb port\n\n" +
        ...
        */
        ...
        {
            return Form1.comPort.doCommand(MAVLink.MAV_CMD.DO_MOTOR_TEST, (float) motor, (float) (byte) thr_type,
        ...
        public void setDigicamControl(bool shot)
        {
            ...
            Form1.comPort.doCommand(MAV_CMD.DO_DIGICAM_CONTROL, 0, 0, 0, 0, 1, 0, 0);
                ...
                if ((message.header == 'U' || message.header == 0xfe || message.header == 0xfd) && buffer.Length >= message.payloadlength)
                {
                    ...
                    lock (objlock)
                    {
                        ...
                        if (msgid == (byte)MAVLink.MAVLINK_MSG_ID.ADSB_VEHICLE)
                        {
                           ...
                           // From1.instance.adsbPlanes[adsb.ICAO_address.ToString("X5")] = new ANOGround.Utilities.adsb.PointLatLngAltHdg(adsb.lat / 1e7, adsb.lon / 1e7, adsb.altitude / 1000, adsb.heading * 0.01f, adsb.ICAO_address.ToString("X5"), DateTime.Now);
                        }
                    }
